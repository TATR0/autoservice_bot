...
| Кто начисляет дни | бот, сразу | владелец бота, командой |
...
**Telegram о таком платеже не узнаёт**, поэтому дни начисляются руками. Чтобы
это не превращалось в расследование, бот в момент выдачи ссылки пишет
владельцу: какой сервис, какой тариф, сколько рублей и готовую команду. Когда
перевод придёт на кошелёк — остаётся её выполнить:

    /extend sub:11111111-1111-1111-1111-111111111111:30

Это метка платежа. ЮMoney возвращает её нетронутой в истории переводов, и
связать деньги с сервисом больше нечем: имя плательщика о сервисе не говорит
ничего. Команда принимает метку целиком — делить строку руками значит однажды
