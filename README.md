...
**Пока приёма денег нет, подписка никого не отключает.** Механизм собран
целиком: срок считается, продления пишутся в журнал, гейты и напоминания
проверены тестами. Но `SUBSCRIPTION_ENFORCED` по умолчанию выключен, и в этом
состоянии бот работает так, будто подписки нет вовсе — ни один сервис не
пропадёт из поиска, ни одно письмо не уйдёт, пробный период ничем не кончится.

Включается одной строкой в день, когда появится оплата:
