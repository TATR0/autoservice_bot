...
1. Счётчик на ключ (IP). Ловит обычное злоупотребление. За обратным прокси
   адрес берётся из X-Forwarded-For — и только если TRUST_PROXY включён:
   без прокси этот заголовок пишет сам клиент, и лимит обходится строкой.
...
def client_key(request: Request) -> str:
    """
    Ключ клиента: адрес соединения, а за прокси — первый адрес из
    X-Forwarded-For.

    Заголовку верим только при TRUST_PROXY. Иначе любой желающий менял бы
    его от запроса к запросу и получал бы свежий лимит каждый раз — то есть
    счётчика не было бы вовсе. Вторая линия обороны (DatabaseGate) от
    заголовков не зависит и работает в обоих случаях.
    """
