...
def tariff_screen(svc) -> str:
    ...
    return f"{head}\n\nВыберите срок продления:{_offer_note()}"


def _offer_note() -> str:
    """
    Ссылка на оферту под тарифами: оплата — и есть согласие с ней, поэтому
    прочесть её можно до оплаты, а не после. Реквизиты не заполнены — оферты
    нет, и вместо ссылки не появляется ничего.
    """
    if not (config.offer_published() and config.BASE_URL):
        return ""
    return (f'\n\n<i>Оплата означает согласие с '
            f'<a href="{config.BASE_URL}/offer">офертой</a>.</i>')


def payment_method_screen(plan) -> str:
    """
    Шапка выбора способа оплаты. Цены — на кнопках, здесь только разница между
    способами: платить звёздами дороже, и узнать об этом лучше до нажатия.
    """
    lines = [f"💳 <b>{plan.label}</b> — чем платите?"]
    if config.STARS_FEE_PCT > 0:
        lines += [
            "",
            "Звёздами выходит дороже: Telegram и магазины берут комиссию с "
            "покупки звёзд. Переводом — обычная цена, но нужна карта "
            "российского банка.",
        ]
    return "\n".join(lines)
...
def pay_support() -> str:
    """
    Ответ на /paysupport. Владельцу бота о вызове уже сообщено, но сам текст
    жалобы бот никуда не пересылает — поэтому и не просим писать её сюда:
    такое сообщение ушло бы в пустоту. Контакт — главное в этом ответе.
    """
    lines = ["💬 <b>Вопрос по оплате</b>", ""]
    contact = config.SUPPORT_CONTACT or config.OFFER_CONTACT
    if contact:
        lines.append(
            f"Напишите, что случилось, сюда: {h(contact)}. Если платили — "
            "укажите, когда и каким способом."
        )
    else:
        lines.append("Мы уже знаем о вашем вопросе и свяжемся с вами в Telegram.")
    lines += [
        "",
        "Звёзды за неиспользованную подписку возвращаются тем же платежом, "
        "перевод через ЮMoney — обратным переводом.",
    ]
    return "\n".join(lines)
...
def payment_target(svc, plan) -> str:
    """
    Назначение перевода — то, что плательщик увидит на форме ЮMoney.

    Без экранирования: строка уходит в параметр ссылки, а не в сообщение, и
    HTML-мнемоники показались бы в ней как есть.
    """
    return f"Подписка «{svc['service_name']}», {plan.label}"


def payment_link_screen(svc, plan) -> str:
    """Экран оплаты переводом. Сама ссылка живёт на кнопке под сообщением."""
    lines = [
        f"💳 <b>{h(svc['service_name'])}</b> — {plan.label}, {plan.rubles} ₽",
        "",
        "Сумма и назначение платежа уже подставлены — остаётся подтвердить "
        "перевод.",
        "",
        # Честно про задержку: перевод приходит не боту, а на кошелёк, и
        # обещать мгновенное продление здесь было бы враньём
        "Подписка продлится после того, как перевод дойдёт. Обычно это "
        "занимает несколько минут.",
    ]
    if config.SUPPORT_CONTACT:
        lines += ["", f"Если через сутки ничего не изменилось — напишите "
                      f"{h(config.SUPPORT_CONTACT)}."]
    return "\n".join(lines)
