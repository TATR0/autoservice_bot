-- ============================================================
-- AutoService Bot — схема базы данных v2 (PostgreSQL)
-- ============================================================
-- Применяется сама: при первом старте контейнера db и на каждом выкате
-- (scripts/deploy.sh). Скрипт идемпотентный: его можно выполнять и на
-- пустой базе, и поверх схемы v1. Вручную: psql "$DATABASE_URL" -f schema.sql
...
-- ── Row Level Security ───────────────────────────────────────
-- Политики не создаются намеренно: бот ходит в базу под владельцем таблиц, и
-- RLS его не ограничивает. Смысл в том, что любой другой роли таблицы не
-- отдадут ни строки — на своей базе это запас, на чужой (Supabase с его
-- публичным anon-ключом) единственное, что закрывает ФИО и телефоны.
