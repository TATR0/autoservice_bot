...
def check_env(env: Mapping[str, str]) -> list[Problem]:
    ...
    # Тоже не стоп: сервис вправе хранить данные клиентов сколько считает
    # нужным. Но выбрать это он должен сам, а не потому, что переменную не
    # заметили: клиент, записавшийся однажды, остаётся в базе с телефоном
    # и госномером навсегда
